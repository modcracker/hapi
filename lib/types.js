...
Types.prototype.validateCollection = function(collection, rules, callback){
  ...
  // var ruleType = this.resolveType(rules);
  var process = (rules instanceof Array) ? this.validateDSL : this.validateNormal;
  
  // switch(ruleType){
  //   case "array":
  //     process = this.validateDSL; // TODO: these process functions need state
  //     break;
  //   case "object":
  //     process = this.validateNormal;
  //     break;
  //   default:
  //     throw "Unexpected type detected for rules in Types.validateCollection(source, rules, callback)";
  //     break;
  // }
  var requiredCount = 0;
  var state = {}
  ...
  for(var i in collection){
    if (typeof rules[i] === "undefined" || rules[i] === null || typeof rules[i] !== "object"){
      continue; // No rules supplied => skip?  TODO: confirm
    }
